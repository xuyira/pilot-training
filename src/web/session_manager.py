...
from src.core.adaptive import decide_next_level
...
class SessionManager:
    ...
    def start_block(self, token: str) -> dict[str, Any]:
        ...
        return {
            "blockId": block_id,
            "durationSeconds": session.config_bundle["base"]["ui"]["placeholder_block_seconds"],
    ...
    def log_event(self, token: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        row = {
            ...
            "module_name": context.selection.module_name,
            "block_id": context.selection.block_id,
            "difficulty_level": context.selection.start_level,
    ...
    def complete_block(self, token: str) -> dict[str, Any]:
        ...
        level = context.selection.start_level
        block_id = context.selection.block_id
        next_level, adapt_action = decide_next_level(
            context.selection.module_name,
            level,
            {"implemented": False},
            session.config_bundle["base"]["adaptive"],
        )
        session.marker_client.send_marker(
            context,
        ...
        session.marker_client.send_marker(context, "block_end", block_id, level)
        summary = build_block_summary(
            ...
            notes="Web placeholder block executed. Module internals pending.",
        )
